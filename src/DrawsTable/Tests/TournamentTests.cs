...
namespace DrawsTable.Tests
{
    public class TournamentTests : IDisposable
    {
        ...
        {
            string path = Directory.GetCurrentDirectory();
            string filePath = path + @"\wwwroot\data\PlayersAndTeams.txt";
            _tournament.LoadPlayersFromTxt(filePath);
        ...
        [Fact]
        public void MakeDrawSeedingWithThe32PlayersMustNotCrash()
        {
            string path = Directory.GetCurrentDirectory();
            string filePath = path + @"\wwwroot\data\PlayersAndTeams.txt";
            _tournament.LoadPlayersFromTxt(filePath);
            _tournament.MakeDraw(DrawMap.ThirtyTwo, 4, 3);
            Assert.Equal((int)DrawMap.ThirtyTwo / 2, _tournament.Matches.Length);
